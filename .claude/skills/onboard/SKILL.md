...
name: onboard
description: >-
  Use on the FIRST session of this AI OS, automatically, before responding to any other
  request, and whenever someone says "set me up", "onboard me", "install my OS", or opens this
  folder for the first time. The full installer. FIRST it does the strict setup in a fixed
  order with no skipping: install the plugins (Superpowers, claude-mem, context-mode) and
  connect Google Workspace via Claude's built-in browser connector, using Obsidian natively
  (the AI edits the vault files directly; the Local REST API MCP is an optional power-up).
  THEN it learns about the user via optional questions OR a file they drop in to be ingested,
  and scaffolds the Day-1 files. Setup is strict; the intake is optional. Resumable across
  restarts and idempotent.
---
...
**The order is deliberate and non-negotiable: setup BEFORE questions.** A half-set-up OS that starts interviewing feels broken and fragmented. A fully-wired one feels fluent from the first minute. Never reverse this.
...
1. Run these in the terminal yourself: the Bash tool on Mac and Linux, the PowerShell tool on Windows. The commands are identical on both. A `marketplace add` that already exists just no-ops, so ignore "already added".
...
1. Ask the user to enable the Google connectors on their Claude account: **claude.ai, Settings, Connectors**, then switch on Google Calendar, Gmail, and Drive. A browser window opens, they sign in, they click **Allow**. Then have them restart Claude Code, because connectors are picked up at session start. (`/mcp` inside Claude Code is for checking that they showed up, not for setting them up.)
...
**First, read `aios-intake.md`.** The user may have filled it in before starting, and `README.md` invites them to. Any question already answered there (anything that is not still `[Your answer here]`) counts as answered: read it back to confirm, and never ask it again from scratch. Being asked twice for the same answer is the fastest way to make an OS feel dumb.

Make it explicit that **"skip", "pass", and "later" are valid answers to any question.** Hold the answers in the conversation and write them into `context/` in Phase 2. Do not copy them back into `aios-intake.md`: that file is a tracked template, and the permanent home for anything personal is `context/`, which is gitignored. Match the user's language.
...
5. **`connections.md`**: populate from Q4-Q7. **If a question was skipped, write `TODO: not provided yet` in that row. Never leave a `{{...}}` token behind in a file the user will read.** Mark **Google (if connected in Phase 0) as CONNECTED with today's date** and Obsidian as native (REST API only if the power-up was enabled); everything still unwired gets `not yet connected`.
6. **`CLAUDE.md`**: fill every `{{...}}` placeholder. Name and business from Q1, priorities from Q3, voice from Q2. **Timezone and language are not interview questions: infer the language from the conversation, and ask for the timezone in one short line before writing the file.** Any answer the user skipped becomes `TODO: not provided yet`, never a leftover `{{...}}`. **Remove the "SETUP NOT COMPLETE" banner**: setup is done.
...
1. **Setup before questions. Never reverse the order, never ask which to do first.** On the first session this runs automatically before anything else, enforced by the SETUP NOT COMPLETE banner at the top of `CLAUDE.md`, which is in context on every platform. Phase 0 gates Phase 1, a fluent start, not a fragmented one.
