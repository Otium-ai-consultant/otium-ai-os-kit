...
name: os-audit
description: >-
  Use when someone asks to run an OS audit, check their OS for stale or outdated data, verify
  that what the manual points at actually exists, find duplicate or bloated folders, clean up or
  organize their project, or says "os audit", "is my setup stale", "run a data audit", "my
  folder is a mess", "cek OS-ku", "ada yang basi nggak", "beresin folderku", or "my AI keeps
  missing things that are there". Read-only drift, freshness, and organization audit of the
  current project. Companion to the audit skill: audit scores whether the OS is built right,
  os-audit checks whether it is still true.
...
- The `audit` skill in this kit is the companion: `audit` scores whether the OS is built right (Four Cs, structural); `os-audit` checks whether it is still true. Run both for the full picture. If `audit` is not installed, skip the reference. This skill stands alone.
