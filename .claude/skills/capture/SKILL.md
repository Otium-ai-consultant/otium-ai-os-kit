...
name: capture
description: >-
  Use when you drop a thought, fact, file, link, or decision and it needs to land in the right
  place, or when you say "capture this", "file this", "where does this go", "sort my Transition
  folder", "empty the drop-zone", "simpen ini", "taruh di mana ini", "catat ini", "masukin ini",
  "rapiin drop-zone", or drop files into Transition/. Classifies the input and routes it to the
  correct operational home (context/, references/, decisions/log.md, projects/, tasks/,
  templates/, archives/, or your connected tools), then confirms. Knowledge from outside sources
  is handed to the wiki skill instead. Keeps Transition/ empty.
---
...
| Knowledge that came from **outside** (an article, a book, a talk, something someone told you) | **hand it to the `wiki` skill** | ingested wiki page with its source |
| How an API / tool / MCP works, figured out by you | `references/{tool}-api.md` | researched-once guide |
| A repeatable process of your own | `references/sops/{process}.md` | SOP |
...
**The layer rule decides the hard cases.** This OS keeps two layers apart, and `capture` owns only one of them:

- **Operational** (yours): what is happening now and what was decided. `context/`, `tasks/`, `projects/`, `decisions/`, `templates/`, `connections.md`.
- **Knowledge** (not yours): what is true and where it came from. Everything under `wiki/`, owned by the `wiki` and `raw-ingest` skills.

If the input will be **acted on**, it is operational, so file it. If it will be **referred back to**, it is knowledge, so hand it to `wiki` instead of filing it yourself. Never store the same fact in both layers. A long article or web clip is never a `capture` job: route it to `Transition/RAW/` for `raw-ingest`.

> Some OS instances grow extra homes (for example `contracts/` or `photos/`). If a folder exists and clearly fits, route there. Don't invent folders that aren't there, see `EXPANSIONS.md`.
...
4. **Verify `Transition/` is empty** at the end (a `README.md`, `RAW/`, or `.gitkeep` may remain).
